...
class calibration(STAGE):
    ...
    def additional_on_load(self):
        self.pcc.arduino_stream.sendCommand(f"<C,{self.setting_dict["auto_pipette_duration"]},0>")
...
class baseline(STAGE):
    ...
    def additional_exit_test(self):
        self.pcc.label_debug.setText(f"{self.pcc.data.quality_test};{self.pcc.data.time_in_stage_seconds};{self.pcc.data.qb_time_running_sec};{self.pcc.data.quality_seg_list[-1]}")
        ...
        if self.pcc.data.quality_test == 0:
            ...
            self.pcc.label_debug.setText(f"a{self.pcc.data.quality_test};{self.pcc.data.time_in_stage_seconds};{self.pcc.data.qb_time_running_sec};{self.pcc.data.quality_seg_list[-1]}")
            ...
            self.pcc.label_debug.setText(f"b{self.pcc.data.quality_test};{self.pcc.data.time_in_stage_seconds};{self.pcc.data.qb_time_running_sec};{self.pcc.data.quality_seg_list[-1]}")
...
class challenge(STAGE):
    ...
    def recovery_test(self):
        self.pcc.label_debug.setText(f"{self.pcc.data.recovery_status}, {self.pcc.data.recovery_bout_duration}, {self.pcc.data.accumulated_recovery_bout_duration}")
        # test for not recovered conditions
        recovery_dict = {
            "SLB": self.pcc.data.SLB > self.setting_dict["slb_trigger"],
            "VF": 60 / self.pcc.data.avg_tt < self.pcc.data.baseline_vf * (self.setting_dict["vf_recovery_thresh"]/100),
            "HR": 60 / self.pcc.data.avg_rr < self.pcc.data.baseline_hr * (self.setting_dict["hr_recovery_thresh"]/100)
        }
        # report back conditions that are blocking a instantaneous recovered status
        self.pcc.data.recovery_status = ", ".join([i for i in recovery_dict if i])
        ...
        if self.pcc.data.recovery_bout_flag is False:
            if len(recovery_dict) == 0: 
                ...
                self.pcc.data.recovery_bout_duration = (datetime.now() - self.pcc.data.recovery_bout_start).seconds
            else:
                self.pcc.data.recovery_bout_flag = False
                self.pcc.data.recovery_bout_start = None
        else:
            if len(recovery_dict) == 0:
                ...
                self.pcc.data.recovery_bout_flag = False
                self.pcc.data.recovery_bout_start = None
                self.pcc.data.accumulated_recovery_bout_duration += self.pcc.data.recovery_bout_duration
                self.pcc.data.recovery_bout_duration = 0
                
        # test if animal meets recovery criteria

        if not self.pcc.data.con_rec_flag and self.pcc.data.recovery_bout_duration >= self.setting_dict["minimum_sustained_recovery"]:
            self.pcc.data.con_rec_flag = True
            self.pcc.logger.info("Consecutive Recovery Detected")
            self.pcc.data.challenge_history[
                self.pcc.data.current_challenge_round
            ]["con_rec"] = self.pcc.data.current_recovery_duration
        if not self.pcc.data.acc_rec_flag and self.pcc.data.accumulated_recovery_bout_duration >= self.setting_dict["minimum_sustained_recovery"]:
            self.pcc.data.acc_rec_flag = True
            self.pcc.logger.info("Accumulated Recovery Detected")
            self.pcc.data.challenge_history[
                self.pcc.data.current_challenge_round
            ]["acc_rec"] = self.pcc.data.current_recovery_duration
        ...
        if (
            ...
            and self.pcc.data.current_recovery_duration >= self.pcc.data.current_recovery_waiting_interval
        ):
            ...
            if self.setting_dict["recovery_mode"] == "consecutive":
                if self.pcc.data.con_rec_flag is True:
                    ...
                    self.pcc.data.current_recovery_waiting_interval += self.setting_dict["recovery_increment"]
                    self.pcc.logger.info(f"animal not yet recovered: rec bout ({self.pcc.data.recovery_bout_flag}), rec status ({self.pcc.data.recovery_status})")
                ...
                self.pcc.data.error_dict["BAD_RECOVERY_SETTINGS"] = {"message":f"setting for recovery mode not among implemented options - {self.setting_dict["recovery_mode"]}"}
    ...
    def apnea_test(self):
        if self.pcc.data.SLB >= self.setting_dict["slb_trigger"]:
            ...
            self.pcc.data.challenge_history[
                self.pcc.data.current_challenge_round
            ]["expose"] = self.pcc.data.current_gas_exposure_duration
            self.pcc.arduino_stream.sendCommand(f"<R,{self.setting_dict["position_ra"]},0>")


        elif (
            self.pcc.data.current_gas_exposure_duration > self.setting_dict["max_gas_exposure"]
        ):
            self.pcc.data.error_dict["RUN_ERROR"] = {"message":"animal not going into apnea, this may be a sign of a leaky mask, an empty tank, or another anomoly that needs to be addressed"}
    ...
    def gasp_test(self):
        ...
        if len(self.pcc.data.breath_list) > 0:
            ...
            self.pcc.data.current_latency_to_gasp = (datetime.now() - self.pcc.data.current_recovery_start).seconds
            if self.pcc.data.current_latency_to_gasp < self.setting_dict["short_recovery_warning"]:
                self.pcc.data.error_dict["SHORT_RECOVERY_WARNING"] = {"message":"recovery gasping detected immediately after initiation of recovery period. this may indicate that the animal did not enter a sustained apnea requiring autoresuscitation."}
                self.pcc.logger.warning(self.pcc.data.error_dict["SHORT_RECOVERY_WARNING"])
            self.pcc.data.challenge_history[
                self.pcc.data.current_challenge_round
            ]["lat_to_gasp"] = self.pcc.data.current_latency_to_gasp
    ...
    def exposure_test(self, timeout = None):
        ...
        """
        if any([self.setting_dict["challenge_phrase"] in i for i in self.pcc.arduino_list]):
            self.pcc.data.challenge_state = "exposure"
            self.pcc.data.challenge_history[
                self.pcc.data.current_challenge_round
            ]["prefill"] = self.pcc.data.current_prefill_duration

        elif timeout is not None:
            if self.pcc.data.current_prefill_duration > self.setting_dict["prefill_limit"] + self.setting_dict["prefill_duration"]:
                self.pcc.data.error_dict["PREFILL ERROR"] = {"message":f"prefill duration exceeded typical timing by {self.setting_dict["prefill_limit"]}, communication with the arduino may have been lost"}
    ...
    def additional_event_loop(self):
        ...
        if self.pcc.data.challenge_state == "recovery":
            ...
            self.pcc.data.current_recovery_duration = (datetime.now() - self.pcc.data.current_recovery_start).seconds
            ...
            if self.pcc.data.new_state is True:
                ...
                self.pcc.data.recovery_bout_duration = 0
                self.pcc.data.accumulated_recovery_bout_duration = 0
                self.pcc.data.current_recovery_waiting_interval = self.setting_dict["minimum_recovery"]
            ...
            self.pcc.data.current_prefill_duration = (datetime.now() - self.pcc.data.current_prefill_start).seconds
            ...
            self.pcc.data.current_gas_exposure_duration = (datetime.now() - self.pcc.data.current_gas_exposure_start).seconds
            
            # after challenge delay, transition to thresh2 (if possible change marker color)
            if not (
                self.pcc.data.flow_thresh_to_use == 2 
                and self.pcc.data.current_gas_exposure_duration > self.setting_dict["threshold_transition_delay"]
            ):
                self.pcc.data.flow_thresh_to_use = 2
        ...
        self.pcc.data.new_state = self.pcc.data.prev_challenge_state != self.pcc.data.challenge_state
    ...
    def additional_exit_test(self):
        if self.pcc.data.SLB > self.setting_dict["call_death_trigger"]:
            ...
            self.pcc.data.current_challenge_round >= self.setting_dict["challenge_round_limit"] 
