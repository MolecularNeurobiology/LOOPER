...
def main():
    ...
    local_storage_path = os.path.join("/media/pi", config["RIGNAME"])
    ...
    # build list of RUIDS
    ruid_list = [
        extract_ruid(v) for k, v in filedict.items() if extract_ruid(v) is not None
    ]
    ...
    fmp_query_by_ruid = [{"RUID": i, "DeleteRecord": "Yes"} for i in ruid_list]
