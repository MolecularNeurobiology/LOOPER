...
from PyQt5.QtGui import QStandardItemModel, QStandardItem, QTextCursor, QFont
from PyQt5.QtCore import QModelIndex, QObject, Signal, QThread, QThreadPool, QRunnable, QTimer
...
class ccac_main_window(Ui_MainWindow):
    def __init__(self, MainWindow, model):
        ...
        self.actionCopy_and_Check.triggered.connect(self.action_copy_and_check_files_worker)
        ...
        self.pushButton_copy_files.clicked.connect(self.action_copy_and_check_files_worker)
        self.pushButton_compare_files.clicked.connect(self.action_compare_files_worker)
        self.pushButton_clear_backed_up_files.clicked.connect(self.action_clear_files_worker)
    ...
    def action_refresh_folder_list(self):
        ...
        self.logger.debug(
            f'output folders updated: {",".join(self.model.output_folder_list)}'
        )
    ...
    def action_copy_files(self):
        for i,f in enumerate(self.model.input_file_list):
            self.logger.info(
                f'working on file {i+1} of {len(self.model.input_file_list)}'
            )
    ...
    def action_copy_and_check_files_worker(self):
        if not self.model.output_folder_list or not self.model.input_file_list:
            ...
            copy_worker = Worker(self.action_copy_files)
    ...
    def action_compare_files_worker(self):
        if not self.model.output_folder_list or not self.model.input_file_list:
            self.logger.warning('unable to copy')
        else:
            ...
            compare_worker = Worker(self.action_compare_files)
    ...
    def action_clear_backed_up_files(self):
        ...
        if not self.model.output_folder_list or not self.model.input_file_list:
            ...
            for f in self.model.input_file_list:
                ...
                self.exit_status.append(f'{os.path.basename(f)} : {exit_status}')
