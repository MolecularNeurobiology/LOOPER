...
def cycler_prep(instance, label):
    ...
    return {
        "TT": {
            1: {"on_limit": TT_on, "beat_limit": TT},
            0: {"on_limit": TT_2_on, "beat_limit": TT_2},
        },
        "RR": {
            1: {"on_limit": RR_on, "beat_limit": RR},
            0: {"on_limit": RR_2_on, "beat_limit": RR_2},
        },
    }
...
def widget_builder(
    ...
    getattr(instance, f"{state}_set").setFixedSize(25, row_increment * len(metrics))
    getattr(instance, f"{state}_set").move(
        start_position[0] + size[0][0] + size[1][0], start_position[1] + row_increment
    )
    ...
    for metric in metrics:
        ...
        setattr(
            instance,
            f"{label}_edit",
            QTextEdit(str(getattr(instance, label)), instance),
        )
        ...
        getattr(instance, f"{label}_edit").move(position[0] + size[0][0], position[1])
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.move(100, 100)
        self.Label_1 = QLabel(
            "<h1>GUI Electronic Mouse - v{}</h1>".format(__version__), parent=self
        )
        ...
        self.state_label.move(0, 650)
        self.state_label.setAlignment(Qt.AlignCenter)
        self.state_label.setText(f"<h1>{self.state}</h1>")

        self.pin_input_reading = QLabel("input pin: N/A", parent=self)
        self.pin_input_reading.setFixedSize(685, 90)
        self.pin_input_reading.move(0, 675)
        self.pin_input_reading.setAlignment(Qt.AlignCenter)
        self.pin_input_reading.setText("input pin: N/A")
