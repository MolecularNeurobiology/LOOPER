...
try:
    ...
    from .rabbitmq_client import RabbitMQClient
try:
    from config import PING_QUEUE, COMMAND_QUEUE, MINERVA_STREAM_QUEUE, STREAM_TIMEOUT_SECONDS
except:
...
class Plugin:
    ...
    def __init__(self, registrationParams: PluginRegistration, logger):
        ...
        self._rabbit_mq_client_consumer = RabbitMQClient(logger, COMMAND_QUEUE, registrationParams.mac_address, use_ttl=False)
        self._rabbit_mq_client_producer = RabbitMQClient(logger, PING_QUEUE, use_ttl=False)
        ...
        self._default_stream_data = MinervaStreamData(
    ...
    def _stream_data_to_server(self, session: UserStreamSession):
        ...
        try:
            ...
            self._log_info(f"Stream data sent to server for user {session.user_id}, MAC {session.stream_data.mac_address}")
                        ...
                        try:
                            ...
                            if command_type_lower in string_command_map:
                                ...
                                try:
                                    ...
                                    self._log_info(f"Attempting to process string command type: {command_type}")
    ...
    def _create_user_session(self, user_id: str, payload: Dict[str, Any], current_time: datetime):
        ...
        self._log_info(f"Creating user session for user {user_id} with rig-specific queue: {user_queue}")
    ...
    def _update_session_stream_data(self, session: UserStreamSession, payload: Dict[str, Any]):
        """
    ...
    def _update_stream_data_from_payload(self, stream_data: MinervaStreamData, payload: Dict[str, Any]):
        """
        ...
        with self._sessions_lock:
            if user_id in self._user_sessions:
                ...
                try:
                    ...
                    self._log_error(f"Error closing RabbitMQ client for user {user_id}: {e}")
        ...
        while self._is_running:
            ...
            with self._sessions_lock:
                ...
                for user_id in users_to_remove:
                    self._log_info(f"Session timeout for user {user_id}, cleaning up...")
    ...
    def is_user_streaming(self, user_id: str):
        ...
        """
        with self._sessions_lock:
            return user_id in self._user_sessions and self._user_sessions[user_id].is_active
